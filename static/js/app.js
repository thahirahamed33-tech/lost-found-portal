...
async function openItemModal(itemId) {
    try {
        ...
        content.innerHTML = `
            ...
            <span class="item-badge ${statusClass}">${capitalize(item.status)}</span>
            <h2>${item.name}</h2>
            ...
            <div class="item-detail-info">
                ...
                <div class="info-item">
                    ...
                    <div class="info-value">${item.user_name || 'Unknown'}</div>
                </div>
                <div class="info-item">
            ...
            <div class="item-detail-actions">
                ${currentUser && currentUser.id !== item.user_id ? `
                    ...
                    ` : ''}
                ` : ''}
                ${item.type === 'lost' && currentUser ? `
                    <button class="btn btn-secondary" onclick="markAsFound(${item.id})">
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
function contactOwner(itemId) {
    showToast('info', 'Contact Info', 'Contact information will be shown after admin approval');
}

function markAsFound(itemId) {
    closeItemModal();
    navigateTo('/report');
    showToast('info', 'Found an Item', 'Please report that you found this item');
}
...
async function submitClaim(event) {
    ...
    try {
        ...
        showToast('error', 'Error', 'Failed to submit claim');
    }
}
...
function createItemCard(item) {
    ...
    return `
        <div class="item-card" onclick="openItemModal(${item.id})" style="cursor: pointer;">
...
function resetReportForm() {
    ...
    document.getElementById('otherLocationGroup').classList.add('hidden');
}
...
function markAllRead() {
    ...
    fetch(`${API_URL}/api/auth/notifications/read-all`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${authToken}` }
    })
